...
## Layer decision: run the resolver in the CONTROLLER (NOT the relay)
The controller is the only layer that already holds all four inputs a policy-aware
...
The VPN data path is a raw Noise tunnel client↔node, NOT client↔controller, so:
1. **Direct mTLS `Resolve` RPC (Phase 1, build first).** The client is already
   on an authenticated mTLS gRPC channel to the controller; add `UserAPI.Resolve`
...
2. **In-overlay resolver IP `100.64.0.53` (Phase 1.5, MagicDNS transparency).**
   ...
   `100.64.0.53:53` up its existing mTLS control stream to the controller resolver
...
- **Phase 1 — machine-name→IP:** new `internal/dns` (`resolver.go` miekg-based +
  `ServeDNS` seam, `zone.go` name parse); `internal/controller/dns.go` (wire
...
- **IMPLEMENTED instead (supersedes the old "Phase 1.5 hairpin"):** in-network DNS
  is resolved LOCALLY at each agent — NOT by querying the controller. The controller
  ...
  (zero per-query controller dependency, offline-safe). The old per-query
...
`internal/controller/broker.go` (the exact policy.Input to mirror), `internal/policy`,
`internal/controller/overlay.go`+`store.go` (stable overlay IPs), `cmd/geneza/vpn.go`+
