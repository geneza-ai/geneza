...
>    (stream GatewayUserMsg)`), not server-streaming — a server-stream has no client→server leg
...
- The proto `api/proto/geneza/v1/control.proto`: `AgentMsg.oneof` (free tag **7**;
  used 1-6 at `:64-71`), `GatewayMsg.oneof` (free tag **7**; used 1-6 at `:88-95`),
...
3. **Blind DERP-lite relay**: a new UDP forwarder co-resident in the existing
   `internal/relay` process, on a separate UDP port (`7404`), keyed by **gateway-minted
...
4. **Gateway control/signaling**: the existing mTLS dial-out streams (`NodeControl.Stream`,
   and the planned `UserAPI.WatchNetworks`) carry the new disco/candidate exchange and
   hole-punch timing. The gateway mints/rotates routing ids and is the rendezvous clock.
...
```
                       GATEWAY (trusted; mTLS; DIAL-OUT only)
        ...
        │   GatewayMsg.disco = 7  : CallMeMaybe / PunchAt                 │
...
- **The relay is payload-blind and identity-blind.** It sees only `{8-byte header with an
  opaque rid, opaque bytes}`. WG payload is AEAD-sealed end to end; rids are gateway-minted
...
bytes 2..7  : routing id (rid) = 48-bit big-endian, opaque, gateway-minted (§3.5)
...
from a source address. Possession of the gateway-minted rid (delivered only over mTLS to the two
legitimate endpoints) *is* the capability:

1. Gateway mints `{ridA, ridB}` for an ordered peer pair `(A,B)` in a VNI plus a 16-byte
...
Central adaptation away from DERP: **the rid is minted by the gateway, NOT derived from a WG
...
without the gateway's private `rid → (VNI,pair)` map, which the relay never sees.
...
the same surface (deployment stays "gateway + relay", no third service):
...
and reports a `nat_class` hint up the control stream so the gateway can decide whether a punch is
even worth attempting (§5.3, §5.4).

### 3.5 Opaque per-Network routing ids + scoping

rids are minted **per (VNI, ordered-pair)** by the gateway, which keeps the authoritative
...
- NAT-rebind survival: the relay records `table[rid].addr = observed source addr` on **every**
  ...
  how DERP tracks live sockets and how the gateway's existing `setObservedIP` trick works.
...
- **RELAY (start/floor)**: on `upOrSyncLocked` the peer comes up `state=relay`; the bind
  ...
  below it.")
- **PROBING**: triggered when the gateway delivers `CallMeMaybe{candidates}` + `PunchAt{t0}`.
...
## 5. Signaling over the gateway control stream + hole-punch
...
(coordination via control server, reachability via disco-over-UDP) but uses the gateway as the
...
DiscoMsg disco = 7;
// GatewayMsg.oneof — free tag 7 (1-6 used at control.proto:88-95):
DiscoMsg disco = 7;

message DiscoMsg {
  uint32 vni        = 1;       // which Network this concerns (VNI-scoped, always)
  bytes  peer_wgpub = 2;       // the OTHER endpoint (gateway fills this on relay)
  oneof body {
...
message CallMeMaybe {                 // gateway -> endpoint
...
message PunchAt {                     // gateway -> both endpoints
  int64  t0_unix_ms = 1;             // coordinated spray start (gateway clock + lead)
  uint32 attempt    = 2;             // retry/backoff accounting
}
message EndpointUpdate {              // endpoint -> gateway (rides AgentMsg.disco / WatchN up-channel)
  uint32 vni        = 1;
...
(`control.proto:79-85`, today just `{vni, listen_port}`). We keep the gateway's existing
...
reflexive + LAN locals as richer candidates. The gateway aggregates all of a peer's candidates
...
The gateway fills `WGPeer.relay` in `networkPeers` (`networkpush.go`) right where it already
fills the direct hint via `s.registry.NodeEndpoint(...)`. rid minting/rotation lives in a new
`internal/gateway/relaypath.go` keyed off `(VNI, selfID, peerID)`, **persisted alongside
`BindingRecord`** so rids survive a gateway restart (otherwise a restart silently reshuffles
...
- **Nodes — `NodeControl.Stream`** (`control.proto:55`, handler `nodecontrol.go:39`): persistent
  ...
  `setWGPort` → `repushAllNetworks`; gateway→agent peer endpoints in `NetworkConfig`). The new
  `DiscoMsg` slots into the **same** `AgentMsg`/`GatewayMsg` oneofs and the **same** dispatch
...
Why overlay-independent: the control streams are mTLS over the gateway's TLS listener
...
### 5.4 Gateway disco router (new)

A `discoRouter` on the gateway:
...
2. On membership/candidate change for a VNI, for each co-member pair `(A,B)` it emits
   `GatewayMsg.disco{CallMeMaybe{B's candidates}}` to A and the mirror to B.
...
1. Both endpoints are on the floor and have reported `EndpointUpdate` (local + reflexive +
   nat_class).
2. Gateway sends `CallMeMaybe{peer candidates}` + `PunchAt{t0}` to both.
...
- **symmetric × symmetric**: reflexive port is unpredictable per-destination → the distributed
  ...
  complexity for the lab/SCS target). Gateway sees both symmetric, skips the punch, the pair
...
| Learn node/user/tenant identity | **No** | rids are gateway-minted random 48-bit tokens unrelated to pubkeys/names; the relay's table is `rid → ephemeral UDPaddr`. No pubkey, name, or VNI ever reaches it. |
...
gateway's private rid↔identity map. This is the DERP trust model, made **stronger on identity**
...
lives only in the gateway and is never serialized to the relay.
...
Workspace (tenant)
  └── Network ── VNI (24-bit; one device.Device gnzw<vni> / one genezaBind / one UDP socket)
        ...
        └── for each ordered member pair (A,B):
              ...
              ├── relay rids {ridA,ridB} minted per (VNI,pair), 48-bit, gateway-private map
...
- rids carry no VNI tag on the wire (relay tenant-blind) but are minted per-VNI and delivered
  only to that VNI's two endpoints. The VNI lives only in the gateway's private map.
...
- **`NetworkConfig` push**: `networkConfigProto`/`pushNodeNetworks`/`repushAllNetworks`/
  `registry.SendNetworkConfig` and the `GatewayMsg_NetworkConfig` oneof — none touch transport.
...
and ends with a specific lab proof. Lab geneza1: bridge `vmbr5` (`10.70.70.0/24`), gateway+relay
...
- **Agent backend (`internal/agentd/wg_userspace.go`)**: `userspaceWGBackend` per §7.2;
  ...
  `dataplane: userspace`.
- **Gateway (`internal/gateway/relaypath.go`)**: mint a deterministic-per-(VNI,pair) rid pair +
...
- **Gateway**: `discoRouter` — handle `AgentMsg.disco{EndpointUpdate}`, aggregate candidates
...
- **Proto**: add `DiscoMsg`/`CallMeMaybe`/`PunchAt`/`EndpointUpdate`; `AgentMsg.disco=7`,
  `GatewayMsg.disco=7`. Worker + nodecontrol dispatch the new oneof.
...
- **Lab proof**: VM 106 (behind `vmbr6` SNAT) ↔ VM 107 (flat). Confirm they start on the floor,
  ...
  (gateway logs "skip punch: both symmetric", ping still works).
...
- **Proto/gateway**: add `rpc WatchNetworks(WatchNetworksRequest) returns (stream NetworkConfig)`
...
- **rid persistence on gateway restart**: if rids aren't persisted, a restart reshuffles every
  flow (both endpoints re-REG, brief floor blip). Mitigation: persist `relaypath` records next to
  `BindingRecord` (P1). Acceptance: kill+restart the gateway, confirm flows recover without
  re-handshake.
...
- **New** `internal/gateway/relaypath.go` — rid minting/rotation/persistence keyed off
...
- **Change** `internal/gateway/networkpush.go` — fill `WGPeer.relay` next to the direct-hint
  logic; add the gateway disco router (handle `DiscoMsg` on `nodecontrol.go` switch + on
  `WatchNetworks`; aggregate `EndpointUpdate`; emit `CallMeMaybe`+`PunchAt`).
- **Proto** `control.proto` — `DiscoMsg` (`AgentMsg.disco=7`, `GatewayMsg.disco=7`; mirror in
