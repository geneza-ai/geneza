...
**Status:** enrollment plane **IMPLEMENTED + end-to-end validated** (2026-06-13);
**access plane (§13) now IMPLEMENTED + live-validated** (2026-06-14) — see
`docs/auth-broker-spec.md` for the full identity-broker design. Humans drive
Geneza by logging into the web console with a Keystone password (cloud dropdown)
or via Horizon **trusted_dashboard** websso, auto-provisioning/joining a
workspace (first human = ws-admin); the CLI logs in by RFC 8628 **device code**
brokered through the console. The enrollment plane runs in
`internal/controller/{openstack,vendordata}.go` + the clouds registry in `config.go`;
...
- **TLS (#3):** Nova reached the endpoint over the public Let's Encrypt front
  (system-trusted); installer fetch vs controller runtime use distinct certs.
...
- **A controller→Keystone/Nova callback** (resolve/verify the instance's project,
...
9. RETURN: {"cloud-init": "#cloud-config\n" + render(cloud-init template)}
   with @PROVIDER@=token, @JOIN_TOKEN@=<minted>, @CONTROLLER_HOST@=<public FQDN>,
...
`internal/controller/openstack.go`, `go get github.com/gophercloud/gophercloud/v2`.
...
#cloud-config
runcmd:
  - curl -fsSL https://<controller>/install.sh | sh -s -- --token <minted> --provider token
...
instance identity and the controller re-verifies, removing the token from metadata.

---

## 13. Access plane (Keystone-token auth) — IMPLEMENTED (2026-06-14)

> **Built + live-validated.** See `docs/auth-broker-spec.md`. The controller is now
> a unified identity broker: it mints an **opaque sha256-hashed session** after
> authenticating a human by Keystone (password form / cloud dropdown, or Horizon
> websso **trusted_dashboard** at `POST /openstack/<svc-uid>`), OIDC, or local —
> all coexisting in one workspace via a store-backed member table. A human's
> project resolves to a workspace by the **same binding** the enrollment plane
> uses (`openstack:project:<svc>:<uuid>`); the first human into an unbound project
> (when `allow_human_auto_provision`) becomes **ws-admin**, others map via the
> least-privilege `role_map` (`member→ws-viewer`). Guards #9 (reject service
> tokens) / #10 (require project scope) are enforced fail-closed. The CLI no
> longer logs in by password — `geneza login` is the **RFC 8628 device grant**,
> approved in the console. `admin` is reserved: a login grants at most ws-admin
> (cluster admin is break-glass-cert-only). Original design sketch below.
...
> namespacing); plus **#6** (leaf-cert revocation: `genezactl cert revoke`) and
...
- **Artifact *forgery* over a hostile channel (§12, TUF-lite):** a compromised/MITM controller "can serve stale-or-junk bytes but cannot forge an artifact the root never authorized"; anti-rollback floor closes the downgrade window.
