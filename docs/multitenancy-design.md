...
- **One controller hosts many workspaces** (not workspace==controller) — lowest
...
- **Shared two-tier CA with workspace in the leaf identity URI**:
  `geneza://user/<workspace>/<name>`, `geneza://node/<workspace>/<id>` (controller/
...
- **Roles are per-workspace** (admin in A grants nothing in B). A rare
  **controller operator/super-admin** (minted via break-glass `IssueUserCert`) is
...
- Controller config with no workspace section == today (one `default` workspace,
...
- **Phase 2 — multiple workspaces + operator mgmt (no UI).** Operator trust
  ...
  `genezactl workspace ...`, `client.Profile.Workspace`. Multi-workspace
  isolation tests.
...
Critical files: `internal/controller/store.go` (the boundary + migration),
`internal/controller/broker.go` (fail-closed scoping), `internal/ca/ca.go`
(workspace in URI), `internal/controller/auth.go`+`identity.go` (server-derived
workspace), `internal/controller/server.go` + `api/proto/.../control.proto`
