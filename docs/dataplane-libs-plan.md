...
**wireguard-go as the WG device** (it is the standard) and keeping Geneza's gateway/policy/
...
- `internal/gateway/relaypath.go` — mints per-pair relay coordinates (rid pair + flow secret).
  **REPLACED by minting coturn-style ephemeral TURN credentials** (HMAC over a shared secret;
  same "gateway mints, relay holds no per-pair state" property, now standards-based).
...
- **Single-session, gateway-signed, agent-re-verified grants** → Tailscale's netmap is persistent
...
opposite: it is a pure NAT-traversal library that **keeps Geneza's gateway authoritative** — the
gateway becomes the ICE signaling channel over the control stream it already owns, and the relay
becomes a TURN server the gateway feeds credentials to. Effort is medium; standards-reuse is
...
| **pion ice/turn/stun (chosen)** | **highest (RFC 8445/5389/5766)** | **good — gateway is the ICE signaller + TURN credentialer** | payload-blind; TURN sees an opaque ephemeral principal + peer 5-tuples (acceptable, §1.4) | medium: per-peer agent lifecycle + signaling adapter + embedded TURN |
| tsnet + Headscale | Tailscale-proprietary (not a standard) | **poor — two control planes** | regression (DERP keyed by node pubkey) | high integration + high re-add of gateway semantics + ongoing Headscale ops |
...
- **`internal/gateway/relaypath.go`** — `relayPathFor`, `relayCoords`, `mintRid`, `mintSecret`,
  the `RelayPathRecord` get-or-create. Replaced by `internal/gateway/turncreds.go`
...
- **The gateway as authoritative coordinator** — it already orders the peer pair (lo/hi) to mint
...
`pion/turn` server in the relay binary; (C) gateway-minted ephemeral TURN credentials + ICE
...
```
  ...
  └───────┬───────┘   over control stream               │
          ...
          ▼                                              │
   gateway control stream  ────────────────────  gateway mints REST creds
...
// iceCreds is what the gateway minted for THIS peer (per §3.2/§3.3).
type peerSetup struct {
	wgPub       [32]byte
	controlling bool          // gateway-assigned (lo=controlling)
...
// ReceiveFunc draining recvCh; port 0 is reported (the gateway uses the control
...
// using the TURN creds + controller role the gateway minted for each.
...
> preserved. That is one explicit, gateway-driven trigger — not a hand-written per-packet SM.
...
// authenticated by gateway-minted coturn-style ephemeral credentials (§3.3). The
...
func newTURNRelay(addr, realm, sharedSecret, publicIP string, log logging.LeveledLogger) (*turnRelay, error) {
	...
	srv, err := turn.NewServer(turn.ServerConfig{
		...
		// shared secret. THIS is the "gateway mints, relay holds no state" model.
...
### 3.3 Gateway-minted ephemeral TURN credentials + ICE signaling

**Credential minting (`internal/gateway/turncreds.go`, replaces `relaypath.go`):** the gateway
...
- **Remote candidate → down:** gateway forwards as `CallMeMaybe{candidates}`; the bind does
...
- **`PunchAt` → "begin/restart ICE":** it no longer sprays pings (ICE's connectivity checks ARE the
  punch). It becomes the gateway's optional "re-run ICE now" trigger → `agent.Restart()` for the
...
message TurnCreds {                  // gateway-minted, ephemeral, relay holds only the secret
  ...
  bool   controlling  = 5;          // gateway-assigned ICE role (lo of the pair)
}
...
- **TURN credentials are minted per `(workspace, vni, peer-pair, session)`** in `turnCredsFor(ws,
  ...
  mailbox) — it does not grant Network membership, which remains enforced entirely by the gateway's
...
- `internal/gateway/turncreds.go` (new) — `turnCredsFor` (§3.3); delete `relaypath.go` and the
...
- `internal/agentd/worker.go` — actually plumb `DiscoMsg` send AND receive (today only
  ...
  `IceCreds`/`PunchAt` down via `discoBackend`. The gateway relays per-peer candidates+creds
...
- **Net LOC across bind+relay+gateway is a wash to a modest reduction** (≈824+~210 in
...
- `internal/vpn/bind.go` (824 lines, the magicsock-lite to delete), `internal/relay/udpforward.go`,
  `internal/gateway/relaypath.go`, `internal/agentd/wg_userspace.go` (the shell to keep),
