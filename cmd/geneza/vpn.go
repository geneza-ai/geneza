...
func newVPNCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Bring up a layer-3 overlay through NODE's named subnet-route or exit-node SERVICE.\n" +
			"Creates a local TUN interface, assigns the controller-allocated overlay IP, and installs\n" +
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if sess.OverlayIP == "" || len(sess.Routes) == 0 {
				return fmt.Errorf("controller returned an incomplete vpn grant (no overlay ip / routes)")
			}

			return runVPN(ctx, sess, api, dnsZone)
		},
	}
	cmd.Flags().StringVar(&dnsZone, "dns-zone", "geneza", "tenant DNS suffix resolved over the VPN (node names)")
	return cmd
}
...
func runVPN(ctx context.Context, sess *client.Session, api genezav1.UserAPIClient, dnsZone string) error {
	...
	// The client end of the overlay sits in the same /24 the controller allocates
	...
	// In-network DNS is resolved LOCALLY from the controller-pushed per-Network zone
	// (the agent resolver at 100.64.0.53) — there is no controller DNS RPC. Wiring the
