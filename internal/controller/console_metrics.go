...
func (c *consoleAPI) handleMetricsQuery(w http.ResponseWriter, r *http.Request, u *consoleUser) {
	if c.s.metrics == nil {
	...
	// Scope to the caller's workspace server-side: a tenant only ever reads its
	// own nodes' series, whatever PromQL it sends.
	data, err := c.s.metrics.QueryInstant(r.Context(), u.Workspace, q, ts)
	if err != nil {
...
func (c *consoleAPI) handleMetricsQueryRange(w http.ResponseWriter, r *http.Request, u *consoleUser) {
	if c.s.metrics == nil {
		promError(w, http.StatusServiceUnavailable, "metrics store disabled")
		return
	}
	...
	data, err := c.s.metrics.QueryRange(r.Context(), u.Workspace, q, start, end, step)
	if err != nil {
		promError(w, http.StatusBadRequest, err.Error())
		return
	}
...
func (c *consoleAPI) handleSetNodeModules(w http.ResponseWriter, r *http.Request, u *consoleUser) {
	...
	if err := c.s.audit.AppendWS(u.Workspace, "agent_modules_set", u.Name, node.ID, "", map[string]string{
