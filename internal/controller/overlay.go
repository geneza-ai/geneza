...
// released on session end; one controller, a few hundred concurrent VPN clients.
...
// per-Network FIB. It reserves the first usable host (.1) as the Network controller
...
func allocIPInCIDR(cidr string, used map[string]bool) (string, error) {
	...
	// Network + the reserved .1 controller are skipped; for a host-bit subnet (/31,
	...
	controller := cloneIP(cur)
	incIP(cur) // skip the reserved .1 controller
	for ipnet.Contains(cur) {
		s := cur.String()
		// Skip the IPv4 broadcast (all-ones host) and the controller.
		if !isBroadcast(cur, ipnet) && s != controller.String() && !used[s] {
			return s, nil
		}
		incIP(cur)
	}
