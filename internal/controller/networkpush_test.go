...
func newDataPlaneServer(t testing.TB) *Server {
	t.Helper()
...
func putWGNode(t testing.TB, srv *Server, ws, id, name string, labels map[string]string, approved bool, wgByte byte) *NodeRecord {
	t.Helper()
	wg := make([]byte, 32)
	wg[0] = wgByte
...
func TestNetworkPeersFilters(t *testing.T) {
	...
	nodes, err := srv.store.ListNodes(ws)
	if err != nil {
		t.Fatal(err)
	}
	peerIPs := srv.networkPeerIPs(ws, prod, self, nodes)
	peers := srv.networkPeers(prod, self, nodes, peerIPs)
	if len(peers) != 1 {
