...
// One interface per Network (VNI); peers + keys come from the controller's
...
// (ReplacePeers: the pushed set is authoritative — peers dropped by the controller
...
// configured with listenPort 0, the kernel picks one; the controller needs it for
// endpoint discovery).
