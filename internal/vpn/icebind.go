...
// multi-peer Bind, and carry ICE signaling over the controller control stream.
...
// controller control stream. The worker implements it via AgentMsg.disco.
...
// PeerSetup is the per-peer ICE/TURN setup the controller minted (from WGPeer.turn).
...
func (b *ICEBind) ensurePeer(s PeerSetup) {
	...
	urls, candTypes, err := icewire.URLs(s.TurnURL, s.TurnUser, s.TurnPass, b.relayOnly)
	if err != nil {
	...
	// Trickle LOCAL candidates up the control stream (controller forwards to the
	...
	// creds+candidates periodically until connected. Re-announce (not a controller
...
// recreatePeer rebuild on either end). The controller holds no signaling state and
...
// AddRemoteCandidate feeds a peer's controller-forwarded ICE candidate to its agent.
...
// without any controller-side signaling state.
...
// OnPunchAt is the controller's "re-run ICE now" trigger (late-upgrade / roam). In
