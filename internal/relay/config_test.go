...
func TestRelaySecretsSynthFromSharedSecret(t *testing.T) {
	p := filepath.Join(t.TempDir(), "relay.yaml")
	if err := os.WriteFile(p, []byte("listen: \":7403\"\ntls: false\nshared_secret: topsecret\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	cfg, err := Load(p)
	if err != nil {
	...
	sec, ok := cfg.Secrets["default"]
	if !ok || sec.Current != "topsecret" {
		t.Fatalf("synthesized secrets = %+v ok=%v", sec, ok)
	}
}
