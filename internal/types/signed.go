...
// Sign marshals payloadObj to JSON and signs it under the given context.
func Sign(priv ed25519.PrivateKey, keyID, context string, payloadObj any) (*Signed, error) {
	payload, err := json.Marshal(payloadObj)
	if err != nil {
		return nil, fmt.Errorf("marshal payload: %w", err)
	}
	sig := ed25519.Sign(priv, signingInput(context, payload))
