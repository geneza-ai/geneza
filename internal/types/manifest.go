...
// OFFLINE by the artifact signing key — never by the controller — and verified
// by the bootstrap against a public key pinned in its own config/build.
// A compromised controller can therefore serve stale manifests at worst, never
// forged ones.
...
// DesiredVersionResponse is returned by the controller's HTTP desired-version
