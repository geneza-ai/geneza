...
// After a reconnect / re-home the controller's per-connection version counter restarts
// at 1, well below a version the agent accumulated on the prior stream. Without a
// reset the agent would silently drop the new controller's re-pushes (the re-home
// convergence bug); resetVersion re-baselines so a fresh-stream push applies and a
// membership change after the move takes effect.
func TestNetworkResetVersionAfterRehome(t *testing.T) {
	m, fake := testNetMgr()
	// Climb the watermark on the first stream.
	for v := int64(1); v <= 10; v++ {
		m.reconcile(&genezav1.NetworkConfig{Version: v, Networks: []*genezav1.NetworkSpec{
			{Vni: 1, OverlayCidr: "100.64.0.2/24", Peers: []*genezav1.WGPeer{peer(1, "100.64.0.3/32")}},
		}})
	}
	if fake.peers["gnzw1"] != 1 {
	...
	// A fresh stream's first push restarts at version 1, adding a co-member. Without
	// the reset this is dropped (1 < 10) and the new peer never applies.
	newOwnerPush := &genezav1.NetworkConfig{Version: 1, Networks: []*genezav1.NetworkSpec{
		{Vni: 1, OverlayCidr: "100.64.0.2/24", Peers: []*genezav1.WGPeer{
			peer(1, "100.64.0.3/32"), peer(2, "100.64.0.4/32")}},
	}}
	m.reconcile(newOwnerPush)
	if fake.peers["gnzw1"] != 1 {
		t.Fatal("precondition: without reset the low-version re-home push is dropped")
	}
	m.resetVersion()
	m.reconcile(newOwnerPush)
	if fake.peers["gnzw1"] != 2 {
		t.Fatalf("after reset, re-home push should apply the new co-member: peers = %d, want 2", fake.peers["gnzw1"])
	}
}
