...
// 100.64.0.53, answering name->overlayIP PURELY from the zone the controller PUSHED
// in NetworkConfig — never querying the controller or relay at lookup time (the
...
import (
	...
	genezadns "geneza.io/internal/dns"
...
// (union); the controller pushes only records this node may resolve.
func (d *dnsResolver) Update(specs []*genezav1.NetworkSpec, managed []*genezav1.DnsZoneSpec) {
	zones := zonesFromSpecs(specs, managed)
	d.mu.Lock()
	d.zones = zones
	d.mu.Unlock()
	if len(zones) > 0 {
...
// zonesFromSpecs merges the pushed NetworkSpecs and the extra managed-domain
// zones into the per-zone record set (union; same suffix merges), longest-suffix
// -first. Managed zones carry no WireGuard interface — they only widen what names
// resolve locally.
func zonesFromSpecs(specs []*genezav1.NetworkSpec, managed []*genezav1.DnsZoneSpec) []*dnsZone {
	byZone := map[string]map[string]dnsRec{}
	add := func(zone string, recs []*genezav1.DnsRecord) {
	...
	for _, sp := range specs {
		add(sp.GetDnsZone(), sp.GetDnsRecords())
	}
	for _, mz := range managed {
		add(mz.GetSuffix(), mz.GetRecords())
	}
