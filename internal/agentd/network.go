...
// controller's desired NetworkConfig: one wg interface per Network (VNI), with the
...
type networkManager struct {
	...
	// per-Network WG listen ports so the worker can forward them to the controller
...
// control stream so the controller can derive a direct endpoint for this node.
...
// resetVersion re-baselines the monotonic version to match a fresh control
// stream. The controller stamps the version from a per-connection counter that
// restarts at 1 on every (re)connect — and on a re-home the new controller's counter
// is unrelated to the old one — so a version accumulated on a previous stream would
// reject every push on the new one. The new stream always re-pushes current state
// (the controller re-derives on connect), so dropping the prior watermark cannot
// regress the running set; only the running interfaces, never reset here, hold
// state across reconnects.
func (m *networkManager) resetVersion() {
	m.mu.Lock()
	m.version = 0
	m.mu.Unlock()
}
...
// brings up / syncs / tears down to match. Monotonic WITHIN a control stream: a
// stale (lower-version) config from the same connection is ignored so a reordered
// re-push cannot regress state. Across a reconnect the watermark is reset (see
// resetVersion) because the controller's per-connection counter restarts.
func (m *networkManager) reconcile(cfg *genezav1.NetworkConfig) {
	...
	// the node's Networks). Local resolution from here on — no controller query.
	if m.dns != nil {
		m.dns.Update(cfg.GetNetworks(), cfg.GetDnsZones())
	}
}
...
// allowedIPs are cryptokey-routing scopes; an endpoint (filled by the controller's
