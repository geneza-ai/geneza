...
// always-connected coordinator both endpoints already hold a control stream to,
// so it forwards each endpoint's ICE candidates + ufrag/pwd to the peer it names
// (the pion/ice signaling channel). It never inspects the candidate strings
// beyond routing; the actual connectivity checks + hole-punch happen agent↔agent
// over UDP. See docs/dataplane-libs-plan.md §3.3.
