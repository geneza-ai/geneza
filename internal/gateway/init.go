...
// InitDataDir provisions a fresh gateway data_dir: CA hierarchy, grant
// signing key, gateway+relay TLS server keypairs, and the initial signed
...
func InitDataDir(cfg *Config) error {
	...
	for _, p := range []pair{
		{cfg.ClusterName, cfg.gatewayCertPath(), cfg.gatewayKeyPath(), ca.KindGateway},
	...
	cc := buildClusterConfig(1, caInst.RootsPEM, keyID, pub, cfg)
