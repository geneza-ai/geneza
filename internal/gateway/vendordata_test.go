...
// tokenFromCloudInit extracts the --token value from the rendered cloud-init the
...
// the attacker-supplied body project-id (security #1).
...
// Nova hits ~5x per boot — all must get the SAME token (security #15).
...
func TestCloudsConfigValidation(t *testing.T) {
	...
	if !cfg.Clouds["a"].RequireNovaServiceToken {
		t.Fatalf("require_nova_service_token must be forced true (security #4)")
	}
	// Duplicate keystone_url across two service-uids is rejected (security #18).
