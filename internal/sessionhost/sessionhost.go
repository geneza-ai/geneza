...
const (
	...
	// maxRecordingCiphertextBytes is the per-recording disk backstop at the agent,
	// matching the controller's upload cap: past it the cast is sealed early and
	// flagged truncated (still a valid, decryptable asciicast).
	maxRecordingCiphertextBytes = 512 << 20
)
...
type host struct {
	...
	// Drain: on SIGUSR1 the host refuses new sessions, warns attached clients, and
	// reports its live active count in the drain-status file so the bootstrap can
	// gate "active==0" (or a bounded window) before it swaps the session-host
	// binary — the same drain-then-swap the relay uses, here so EVERY geneza
	// component (not just the worker) gets updated.
...
func (h *host) Create(ctx context.Context, req *genezav1.HostCreateRequest) (*genezav1.HostCreateResponse, error) {
	// Once draining, refuse new sessions so the active count can fall to zero and
	// the bootstrap can swap the binary. In-flight sessions are unaffected.
	if h.draining.Load() {
		return nil, status.Error(codes.Unavailable, "session host is draining for update; reconnect shortly")
	}
	pol := h.currentPolicy()
	if req.Detachable {
		if pol.forbidDetach {
...
func (h *host) startSession(req *genezav1.HostCreateRequest, u *user.User, pol *policySettings) (*session, error) {
	...
	// Initialize the caps cell permissive (the grant already authorized this
	// session and the agent validated it at connect); realtime deltas only tighten
	// it via SetCaps. Non-nil from birth so handleInput/reattach never read nil.
	s.caps.Store(&genezav1.SessionCaps{
		Allow: true, AllowPty: req.Pty, AllowInput: true,
		AllowSftpWrite: true, AllowNewChannels: true,
	})
	if req.Record {
		// The full recipient list supersedes the single recipient; an older worker
		// that sends only audit_recipient yields the one-element legacy set.
		recipients := req.GetAuditRecipients()
		if len(recipients) == 0 && req.GetAuditRecipient() != "" {
			recipients = []string{req.GetAuditRecipient()}
		}
		// No recipient configured -> record plaintext (the recorder handles the
		// empty set). Recording no longer fails the session closed; confidentiality
		// at rest is the operator's choice via whether an audit recipient is set.
		rec, err := newRecorder(h.spoolDir, hostID, cols, rows, term, shellName,
			recipients, maxRecordingCiphertextBytes, recorderMeta{
				SessionID:   req.SessionId,
				...
				Subject:     req.Subject,
				Action:      req.Action,
				AuditKeyID:  types.AuditKeyID(recipients),
			})
		if err != nil {
			return nil, status.Errorf(codes.Internal, "start recording: %v", err)
		}
		s.rec = rec
	}
...
// SetCaps is the realtime PEP downgrade pushed by the agent (after it has
// verified the signed, downgrade-only delta). It overwrites the session's caps
// cell last-write-wins — the host tracks no epoch because the agent's
// VerifyDowngrade + monotonic-epoch check is the authority before it ever calls
// here. Idempotent: NotFound on an unknown/exited session (the agent logs and
// moves on, e.g. an owed downgrade for a session the host already reaped).
//
// Enforcement split: the host is AUTHORITATIVE for read-only INPUT (handleInput
// gates the PTY/pipe write — the one thing only the host can do for a detached
// session). The other bits (allow_sftp_write, allowed_forwards, allow_new_channels)
// are stored here for audit/future host-side enforcement but are enforced TODAY by
// the agent's SSH handlers (serveSFTP/serveForward/serve*), which own those
// channels — the host never serves sftp or forwards in v1. allow_pty is immutable
// per session (set at Create from the grant) so it is not part of a downgrade.
func (h *host) SetCaps(ctx context.Context, req *genezav1.HostSetCapsRequest) (*genezav1.HostEmpty, error) {
	s := h.lookup(req.GetHostSessionId())
	if s == nil {
		return nil, status.Errorf(codes.NotFound, "no such host session %q", req.GetHostSessionId())
	}
	s.caps.Store(&genezav1.SessionCaps{
		Allow:            true, // a full cut comes as Kill, not a caps downgrade
		AllowInput:       !req.GetReadOnly(),
		AllowNewChannels: req.GetAllowNewChannels(),
		AllowSftpWrite:   req.GetAllowSftpWrite(),
		ForwardTargets:   req.GetAllowedForwards(),
	})
	h.log.Info("session caps set", "host_session", req.GetHostSessionId(), "read_only", req.GetReadOnly())
	return &genezav1.HostEmpty{}, nil
}
...
// supervises us).
func Run(version, socketPath, spoolDir, drainStatusFile string) error {
	...
	defer stop()
	go h.reaper(ctx)
	// Drain on SIGUSR1 and continuously report the active count so the bootstrap
	// can gate a session-host swap on "active==0" within its drain window.
	go h.watchDrainSignal(ctx)
	go h.runDrainStatusFile(ctx, drainStatusFile)
