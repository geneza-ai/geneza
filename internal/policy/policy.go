...
type Engine interface {
	...
	// Records reports whether the policy records any session — record defaults to
	// true per rule unless explicitly set false, so this is false only when every
	// rule turns recording off. The console keys the Recordings UI off it.
	Records() bool
}
...
type Decision struct {
	...
	AllowDetach   bool
	Record        bool
	// ReadOnly is a continuous downgrade signal: the access is allowed but input
	// must be denied (read-only shell / no sftp write). The controller turns this into
	// a signed SessionPolicyDelta on a live session, enforced per operation. It
	// merges MOST-RESTRICTIVE across matching rules — a permissive rule cannot widen
	// a read-only one for the same action.
	ReadOnly bool
	// RequirePresence demands a continuous human-presence factor (heartbeat) for the
	// session; when the factor goes stale the continuous sweep drops it. Like
	// ReadOnly (and require_native) it merges MOST-RESTRICTIVE — a broad allow rule
	// cannot silently disable presence on a sensitive target.
...
type Rule struct {
	...
	// Optional clock window (controller-local time).
	...
	// Record: nil = true (sessions matching this rule are recorded). Set false to
	// exempt a target from recording. Merges MOST-PERMISSIVE for audit coverage: if
	// any matching rule wants recording, the session is recorded.
	Record *bool `yaml:"record,omitempty"`
	// ReadOnly downgrades a matching session to read-only (input denied) while it
	// is live: access is still allowed, but the controller pushes a read-only delta.
	// ReadOnlyTimeWindow, if set, applies read-only only DURING that clock window
	// (e.g. read-only during a maintenance window). Either makes the rule impose
	// read-only; the most-restrictive merge means any matching read-only rule wins.
	ReadOnly           bool        `yaml:"read_only,omitempty"`
	ReadOnlyTimeWindow *TimeWindow `yaml:"read_only_time_window,omitempty"`
	// RequirePresence demands a continuous-presence factor for sessions this rule
	// allows (a sensitive target). Most-restrictive merge: any matching require rule
	// wins, like require_native.
	RequirePresence bool `yaml:"require_presence,omitempty"`
...
// Records reports whether any rule in any role records sessions (record is on by
// default; only an explicit record:false turns it off).
func (s *Static) Records() bool {
	for _, role := range s.doc.Roles {
	...
	for _, rname := range roles {
		...
		for _, rule := range role.Allow {
			...
			d := Decision{
				...
				Record:          rule.Record == nil || *rule.Record,
				ReadOnly:        rule.ReadOnly || (rule.ReadOnlyTimeWindow != nil && rule.ReadOnlyTimeWindow.contains(now)),
				RequirePresence: rule.RequirePresence,
			}
			if !best.Allow {
				best = d
				continue
			}
			// Merge across matching rules: most permissive for access/detach/record/TTL,
			// but most RESTRICTIVE for read-only (any matching read-only rule wins).
			if d.AllowDetach {
			...
			if d.RequirePresence {
				best.RequirePresence = true
			}
