...
// lookup closure, so this package stays free of controller/ca/policy imports (no cycle).
//
...
// controller PUSHED — never querying the controller/relay at lookup time (zero-trust,
...
// (<machine>.<zone>), else ok=false. Single-label only for now.
