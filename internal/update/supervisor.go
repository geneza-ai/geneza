...
// Signal sends sig to the live child, if any. It is used to DRAIN a relay worker
// (SIGUSR1) before a swap — distinct from Stop, which SIGTERMs and waits — so the
// relay sheds traffic and its sessions migrate off while it keeps serving in-flight
// flows. A no-op when no child is running. Returns whether a signal was delivered.
