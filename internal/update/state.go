...
// controller's desired version moves to a different value.
type State struct {
	...
	// built before this, so a compromised controller cannot replay an old, validly
	...
	// controller cannot replay a stale root-keys to revive a retired/compromised
...
// ResetBadOnChange clears the bad list when the controller's desired version
