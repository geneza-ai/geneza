...
type Installer struct {
	...
	ControllerURL string       // base URL, e.g. https://gw.example:7402
...
// BinaryName is the on-disk worker filename for a product: the binary is
// stored at <VersionsDir>/<version>/<product>, so the same install/swap/prune
// machinery serves any product (geneza-agent or geneza-relay) with the only
// difference being which executable name it writes. An empty product is treated
// as geneza-agent for backward compatibility.
func (ins *Installer) BinaryName() string {
	if ins.Product == "" {
		return "geneza-agent"
	}
	return ins.Product
...
// <VersionsDir>/<version>/<product>. Returns the installed binary path
...
func (ins *Installer) Install(ctx context.Context, sm *types.Signed) (string, *types.Manifest, error) {
	...
	tmp := filepath.Join(dir, "."+ins.BinaryName()+".tmp")
	...
	final := filepath.Join(dir, ins.BinaryName())
...
func (ins *Installer) download(ctx context.Context, m *types.Manifest, tmp string) error {
	url := strings.TrimRight(ins.ControllerURL, "/") + m.BlobPath()
	...
	// Cap the read at size+1 so a hostile controller cannot fill the disk;
