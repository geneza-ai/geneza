...
// DesiredPath is the gateway HTTP endpoint the bootstrap polls
...
// artifact downloads. When caRootsFile is set, the gateway's TLS chain is
...
func NewHTTPClient(caRootsFile string, log *slog.Logger) (*http.Client, error) {
	...
	if caRootsFile != "" {
		...
		if log != nil {
			log.Warn("SECURITY: no ca_roots_file configured — gateway TLS is UNVERIFIED; " +
...
// FetchDesired polls the gateway for this node's desired worker version.
// Returns (nil, nil) when the gateway has nothing for us (204, empty body,
// or empty version) — the caller treats that as "converged, do nothing".
func FetchDesired(ctx context.Context, c *http.Client, gatewayURL, node, current string) (*types.DesiredVersionResponse, error) {
	q := url.Values{}
	q.Set("node", node)
	q.Set("current", current)
	u := strings.TrimRight(gatewayURL, "/") + DesiredPath + "?" + q.Encode()
