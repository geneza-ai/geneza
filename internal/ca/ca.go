...
// controller. Leaf identities are encoded as URIs:
//
...
//	geneza://controller/<name>               controller TLS server cert (no workspace)
...
type Identity struct {
	Kind string // node|user|controller|relay
	...
	// resolve to "default" for node/user kinds; controller/relay carry no workspace.
	Workspace string
	Name      string // node id / username / service name
	Roles     []string
	Provider  string // identity provider that authenticated the user
	// Subject is the STABLE provider subject id (keystone user-id / oidc `sub`;
	// local = username). It is the durable authorization key — display names are
...
//	dir/issuing-ca.{key,crt}   — loaded by the controller at runtime
...
func Init(dir, clusterName string) error {
	...
	// Runtime files the controller actually loads (Load reads only these): the
	...
	// The ROOT PRIVATE KEY must not live next to the running controller: filesystem
	// compromise of the controller would otherwise become fleet-wide identity
	...
	// store) and deleting it here. Load() never reads it — the controller runs on
	...
	notice := "SECURITY: this directory holds the Geneza ROOT CA PRIVATE KEY.\n" +
		"Move it OFF this controller host (HSM / KMS / air-gapped store) and delete it here.\n" +
		"The controller runs on issuing-ca.key alone; the root is needed only to rotate the CA.\n"
...
// Load opens the issuing CA from dir (as written by Init), reading the issuing
// key from issuing-ca.key on disk. This is the file-backed default.
func Load(dir string) (*CA, error) {
	signer, err := keysource.Open(keysource.Spec{Path: filepath.Join(dir, "issuing-ca.key")})
	if err != nil {
		return nil, fmt.Errorf("issuing-ca.key: %w", err)
	}
	return open(dir, signer)
}

// Open opens the issuing CA from dir but draws the issuing signer from src
// instead of always reading issuing-ca.key. The default Spec (empty backend)
// reads issuing-ca.key and is identical to Load; a pkcs11 Spec backs the issuing
// key by a token, so the private key never enters this process. The issuing cert
// and trust bundle are always read from dir — only the private key moves.
func Open(dir string, src keysource.Spec) (*CA, error) {
	if src.Backend == "" || src.Backend == keysource.BackendFile {
		if src.Path == "" {
			src.Path = filepath.Join(dir, "issuing-ca.key")
		}
	}
	signer, err := keysource.Open(src)
	if err != nil {
		return nil, fmt.Errorf("issuing CA key source: %w", err)
	}
	return open(dir, signer)
}

// open assembles a CA from the on-disk issuing cert + trust bundle and a signer
// resolved by the caller (file or token).
func open(dir string, signer crypto.Signer) (*CA, error) {
	...
	return &CA{Cert: cert, Signer: signer, RootsPEM: roots, IssuingPEM: certPEM(cblk.Bytes)}, nil
}
...
type Profile struct {
	Kind      string // node|user|controller|relay
	Workspace string // tenant for node/user certs; empty for controller/relay
	Name      string
	TTL       time.Duration
...
func (c *CA) issue(pub any, p Profile) ([]byte, error) {
	...
	case KindRelay:
		// A relay is BOTH a TLS server (endpoints dial its rendezvous floor with
		// this cert) and a TLS client (it authenticates its registrar heartbeat to
		// the controller gRPC with the SAME cert), so it needs both EKUs — a
		// server-only cert is rejected by the controller's client-cert verification.
...
// IssueServerKeypair generates a fresh key and server cert (controller/relay
...
func PeerIdentity(cert *x509.Certificate) (*Identity, error) {
	for _, u := range cert.URIs {
		...
		// Path is /<name> (controller/relay, legacy) or /<workspace>/<name>
